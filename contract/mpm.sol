...
contract MPM is Ownable, SeroInterface {
    ...
    function details() public returns (string json) {
        ...
        (string memory lcode, string memory rcode) =
            codeService.encode(self.id);
        (string memory lpcode, string memory rpcode) =
            codeService.encode(self.parentId);
    ...
    function _beforeUpdate() internal {
        if (!Utils.sameDay(now, lastUpdated)) {
            ...
            if (winnersLen > 0) {
                ...
                for (uint256 i = 0; i < winnersLen; i++) {
                    uint256 winProfit =
                        winnerProfit.mul(winnerRates[i]).div(100);
    ...
    function register(string memory code) internal {
        ...
        uint256 index = investors.length;
        (string memory lchildsCode, string memory rchildsCode) =
            codeService.encode(index);
